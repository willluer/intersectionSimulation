...
w = 600;
h = 600;
canvas = tk.Canvas(root, width=w, height=h, borderwidth=0, highlightthickness=0, bg="gray")
canvas.grid()
...
def _draw_streets(self, laneWidth):
    #vertical lane
    self.create_line(w/2-laneWidth, h, w/2-laneWidth, h/2+laneWidth, fill="black", width=4)
    self.create_line(w/2+laneWidth, h, w/2+laneWidth, h/2+laneWidth, fill="black", width=4)
    self.create_line(w/2-laneWidth, 0, w/2-laneWidth, h/2-laneWidth, fill="black", width=4)
    self.create_line(w/2+laneWidth, 0, w/2+laneWidth, h/2-laneWidth, fill="black", width=4)
    self.create_line(w/2, 0, w/2, h/2-laneWidth, fill="yellow", width=2, dash=(8, 16))
    self.create_line(w/2, h/2+laneWidth, w/2, h, fill="yellow", width=2, dash=(8, 16))
    #horizontal lane
    self.create_line(0, h/2-laneWidth, w/2-laneWidth, h/2-laneWidth, fill="black", width=4)
    self.create_line(0, h/2+laneWidth, w/2-laneWidth, h/2+laneWidth, fill="black", width=4)
    self.create_line(w/2+laneWidth, h/2-laneWidth, w, h/2-laneWidth, fill="black", width=4)
    self.create_line(w/2+laneWidth, h/2+laneWidth, w, h/2+laneWidth, fill="black", width=4)
    self.create_line(0, h/2, w/2-laneWidth, h/2, fill="yellow", width=2, dash=(8, 16))
    self.create_line(w/2+laneWidth, h/2, w, h/2, fill="yellow", width=2, dash=(8, 16))
